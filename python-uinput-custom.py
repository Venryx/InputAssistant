...
# v-replaced
#from .ev import *
origPackagePath = os.path.dirname(os.path.realpath(uinput.__file__))
'''import imp
#ev = imp.load_source('module.name', dir_path + '/ev.py')
__import__(origPackagePath + '/ev.py', globals=globals())'''
import sys
sys.path.append(origPackagePath)
from ev import *
...
_libsuinput_path = os.path.abspath(os.path.join(origPackagePath, "..", "_libsuinput" + sysconfig.get_config_var("SO")))
...
class Device(object):
    ...
    def __init__(self, events, name="python-uinput",
        ...
        for ev_spec in self.__events:
            ...
            #_libsuinput.suinput_enable_event(self.__uinput_fd, ev_type, ev_code) # v-removed
            if len(ev_spec) > 2:
        ...
        #_libsuinput.suinput_create(self.__uinput_fd, ctypes.pointer(user_dev)) # v-removed
